...
void drawObject() {
  ...
  translate(width/2,height-height*0.074); // moves the starting coordinats to new location
  strokeWeight(20);
  stroke(192, 57, 43); // red color (FLAT DESIGN)
...
void drawLine() {
  ...
  //strokeWeight(5);
  //point((height-height*0.37)*cos(radians(iAngle)),-(height-height*0.37)*sin(radians(iAngle)));
  popMatrix();
}
void drawText() { // draws the texts on the screen
  ...
  textSize(40);
  text("A portée : " + noObject, width-width*1, height-height*0.015); //0.875
  text("Angle : " + iAngle +" °", width-width*0.60, height-height*0.015);
